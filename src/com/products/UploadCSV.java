...
public class UploadCSV extends HttpServlet {
		...
		try{
			...
			//csvFields = new ArrayList<String>(Arrays.asList(nextLine));
		     ...
		     while ((nextLine = csvReader.readNext()) != null) {
		    	 ...
		    	 //if dist_sku exist in the product table for vendor, update
		    	 
		    	 Boolean update = checkProduct("PETRA", csvFieldsMap.get(mappingMap.get("DIST_SKU")) != null ? nextLine[csvFieldsMap.get(mappingMap.get("DIST_SKU"))]: "");
	    		 if(update){
					 ...
					 ps.setDouble(17, Double.valueOf(csvFieldsMap.get(mappingMap.get("LENGTH")) != null && !nextLine[csvFieldsMap.get(mappingMap.get("LENGTH"))].isEmpty() ? nextLine[csvFieldsMap.get(mappingMap.get("LENGTH"))] : "0")); //LENGTH, 
					 ps.setDouble(18, Double.valueOf(csvFieldsMap.get(mappingMap.get("WIDTH")) != null && !nextLine[csvFieldsMap.get(mappingMap.get("WIDTH"))].isEmpty() ? nextLine[csvFieldsMap.get(mappingMap.get("WIDTH"))] : "0")); //WIDTH, 
					 ps.setDouble(19, Double.valueOf(csvFieldsMap.get(mappingMap.get("HEIGHT")) != null && !nextLine[csvFieldsMap.get(mappingMap.get("HEIGHT"))].isEmpty() ? nextLine[csvFieldsMap.get(mappingMap.get("HEIGHT"))] : "0")); //HEIGHT, 
					 ps.setDouble(20, Double.valueOf(csvFieldsMap.get(mappingMap.get("MAP")) != null ? nextLine[csvFieldsMap.get(mappingMap.get("MAP"))] : "0")); //MAP, 
					 ...
					 ps.setDouble(22, Double.valueOf(csvFieldsMap.get(mappingMap.get("ESTIMATED_SHIP_WEIGHT")) != null ? nextLine[csvFieldsMap.get(mappingMap.get("ESTIMATED_SHIP_WEIGHT"))] : "0")); //ESTIMATED SHIP WEIGHT, 
					 ...
					 ps.setDouble(26, Double.valueOf(csvFieldsMap.get(mappingMap.get("MSRP")) != null ? nextLine[csvFieldsMap.get(mappingMap.get("MSRP"))] : "0")); //MSRP, 
	...
	public static HashMap<String,String> existingProdMap;
	public Boolean checkProduct(String vendor, String dist_sku){
		Boolean exists = false;
		if(existingProdMap == null){
			try{
				existingProdMap = new HashMap<String,String>();
				Connection con=ConnectionManager.getConnection();
				PreparedStatement ps = con.prepareStatement("select dist_sku,id from products where vendor = \'" + vendor + "\'");
				ResultSet rs=ps.executeQuery();
				while(rs.next()){
					existingProdMap.put(rs.getString("dist_sku"), rs.getString("id"));
				}
			}catch(Exception e){
				e.printStackTrace();
			}
		}else{
			if(existingProdMap.get(dist_sku) != null){
				exists = true;
			}
		}
